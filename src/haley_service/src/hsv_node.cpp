...
public:
    ...
    try {
        ...
        // 将 HSV 图像编码为 JPEG 格式的字节数组
        std::vector<uchar> hsv_buffer;
        cv::imencode(".jpg", hsv_image, hsv_buffer);

        // 将编码后的 HSV 图像数据发送给已连接的客户端
        for (auto hdl : connected_clients) {
            try {
                ws_server.send(hdl, hsv_buffer.data(), hsv_buffer.size(), websocketpp::frame::opcode::binary);
                ROS_INFO("Sent HSV image to client");
            } catch (const websocketpp::lib::error_code &e) {
                ROS_ERROR("Error sending HSV image to client: %s", e.message().c_str());
            }
        }
